...
The POC is accepted only when automated evidence demonstrates all ten scenarios.
...
4. Both variants run through the real quant-assistant formal engine with working long/short orders, signed positions, covering, and a per-side fee of `0.0006` included in trades and metrics.
5. The QuantBT-derived Run Detail renders the real Run, orders, equity, metrics, logs, provenance, and gate outcomes.
